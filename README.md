...
[![Generated by Google Antigravity](https://img.shields.io/badge/Generated%20by-Google%20Antigravity-4285F4?style=flat-square&logo=google)](https://github.com/mzia/PulsarKey)

> **Hardware-backed FIDO2 & Biometric Security Suite for Pop!_OS COSMIC & macOS**  
> *Universal FIDO2 support (YubiKey, Nitrokey, SoloKeys, Feitian, Titan), Apple Touch ID & WebAuthn compatibility, interactive TUI dashboard, native macOS Menu Bar companion, passwordless sudo, and Presence Sentinel auto-lock.*

> [!NOTE]  
> **Code Attribution**: All code in this repository was generated by **Google Antigravity**. The repository maintainer does not claim credit for the implementation.
...
3. On macOS: Automatically configure `/etc/pam.d/sudo_local` to persist across OS updates, pairing Apple Touch ID with your hardware keys.
4. Optionally pair a secondary backup key.
5. Safely configure `sudo`, lockscreen, and administrative elevation.

---

### 3. Launch PulsarKey, Menu Bar Applet, or Sentinel Daemon
...
| `pulsarkey status` | User | Displays comprehensive security status, Touch ID WebAuthn, & hardware health |
| `pulsarkey applet` | User | Runs native Apple Menu Bar companion (macOS) or COSMIC panel applet (Linux) |
| `pulsarkey daemon [action]` | User | Manages background Sentinel service (`install`, `uninstall`, `status`) |
| `pulsarkey touchid [action]` | User/Root | Manages Apple Touch ID WebAuthn integration (`enable`, `disable`, `status`) |
...
│   │   ├── mod.rs
│   │   └── macos/PulsarKeyBar.swift           # Native Apple Menu Bar companion applet (AppKit)
...
> **Notice**: All code in this repository was generated by **Google Antigravity**. The repository maintainer does not take credit for the codebase.
