...
### Ready-Built Package:
`dist/pulsarkey_1.4.1_amd64.deb`

### Installation Methods:
- **COSMIC Store / Eddy**:
  Right-click `pulsarkey_1.4.1_amd64.deb` in COSMIC Files -> **Open With -> Eddy / COSMIC Store** -> Click **Install**.
- **CLI / APT**:
  ```bash
  sudo apt install ./dist/pulsarkey_1.4.1_amd64.deb
  ```

### What the `.deb` package installs:
- `/usr/bin/pulsarkey`: The compiled Rust manager & applet (with `/usr/bin/cosmic-fido2` symlink)
- `/usr/bin/pulsarkey-settings`: Deprecation redirect stub (replaces legacy GUI)
